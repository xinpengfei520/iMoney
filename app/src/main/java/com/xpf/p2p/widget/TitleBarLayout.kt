...
import com.xpf.p2p.databinding.CommonTitlebarBinding
...
class TitleBarLayout @JvmOverloads constructor(
    ...
    private val binding: CommonTitlebarBinding
    ...
    init {
        ...
        binding.tvTitleName.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextSize.toFloat())
        binding.tvTitleName.setTextColor(mTextColor)
        binding.llTitleBg.setBackgroundColor(mTitleBgColor)
        binding.ivMenu.visibility = if (mMenuVisible) VISIBLE else INVISIBLE

        // 设置状态栏占位高度
        if (context is Activity) {
            binding.statusBarSpace.layoutParams.height = StatusBarUtils.getStatusBarHeight(context as Activity)
        }

        binding.ivBack.setOnClickListener { (context as Activity).finish() }
        binding.ivMenu.setOnClickListener { mListener?.onClick() }
    }
